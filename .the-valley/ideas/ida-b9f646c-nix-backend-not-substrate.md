...
- **Verification** — the junction. Checks-as-derivations is the reference implementation, not the
  ...
  **purity as a claim tied to a runner kind** (`nix` claims purity strongly; a `plain`/`oci` runner
  claims less). If derivation-ness leaks into the schema, non-Nix support becomes a migration
  instead of an added backend.

Review heuristic: every schema must be implementable by a shell script on Debian. A field only Nix
...
reference-implementation-not-contract, and align [verification.md](../../design/verification.md).

## Related

- [[ida-1ec03b1]] — path-scoped verification policy; the runner-kind claim is what its per-class
  attestations are made of.
