...
1. **Never losable** (S1). Repos live on infrastructure the owner controls, with no vendor lock-in: daily life stays clone-edit-push, and the platform fades to a mirror nobody thinks about. Project knowledge (outcomes, ideas, decisions, threads) travels with the repo, protected by the same motion that protects the code. Precisely what this means: [durability](#durability), below.
