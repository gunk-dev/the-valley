...
4. `predicate` — the claim, in the order the claim reads. Per predicate type:
   ...
   checks, `predicate.checks` carries one entry per check and `predicate.required` is absent. When
   it required none, `predicate.required none` is the line and `predicate.checks` is absent. There
   is no third way, and in particular no empty list: an array with no elements has no lines, so it
   has no written form, and a landing may not depend on a container the form cannot write down. A
   policy that asks nothing of a diff is an ordinary outcome, so the zero case is said in a line
   rather than left to be inferred from a missing one.
